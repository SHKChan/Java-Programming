...
public class FirstRatings {
     ...
     */
    ArrayList<EfficientRater> loadRaters(FileResource fr) {
        ...
        for (CSVRecord rec : fr.getCSVParser()) {
            ...
            if (null == findRater) {
                EfficientRater newRater = new EfficientRater(rid);
    ...
    void testLoadMovies() {
        ...
        for (String director : directorMoviesMap.keySet()) {
            ArrayList<String> movies = directorMoviesMap.get(director);
            if (max == movies.size()) {
                System.out.println("Director has directed the maximum number of movies: " + director + " - " + movies.size());
            }
        }
    }
    ...
    void testLoadRater() {
        FileResource fr = new FileResource();
        ArrayList<EfficientRater> raterList = this.loadRaters(fr);
        /*
        ...
        String mid = "1798709";
        ArrayList<EfficientRater> tempRaters = new ArrayList<EfficientRater>();
        for (EfficientRater rater : raterList) {
            if (rater.hasRating(mid)) {
                tempRaters.add(rater);
            }
        }
        System.out.println("Movie-" + mid + " has " + tempRaters.size() + " raters.");
        ...
        for (EfficientRater rater : raterList) {
            for (String item : rater.getItemsRated()) {
                if (!tempMovies.contains(item)) {
                    tempMovies.add(item);
                }
            }
        }
        System.out.println("All raters have rated " + tempMovies.size() + " movies.");
    }
